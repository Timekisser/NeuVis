...
from datascts import get_seg_shapenet_dataset
...
class SegSolver(Solver):
  ...
  def model_forward(self, batch):
    ...
    viewdirs = self.embedder(points.normals)
    logit = self.model(data, octree, octree.depth, query_pts, viewdirs) # add points.normal
    labels = points.labels.squeeze(1).reshape(-1)
